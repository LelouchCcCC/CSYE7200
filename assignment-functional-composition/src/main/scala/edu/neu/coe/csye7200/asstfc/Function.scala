...
object Function {
   ...
   */
  // TO BE IMPLEMENTED
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] = (t1y,t2y) match {
  ...
  // TO BE IMPLEMENTED 
{
  ...
  // TO BE IMPLEMENTED 
  {
    ...
    case (Failure(exception), _,_) => Failure(exception)
    case (_,_, Failure(exception)) => Failure(exception)
  ...
  // TO BE IMPLEMENTED
  {
    ...
    case (Failure(exception), _, _,_,_,_,_) => Failure(exception)
    case (_, Failure(exception) ,_,_,_,_,_) => Failure(exception)
    case (_,_, Failure(exception), _,_,_,_) => Failure(exception)
    case (_,_,_,Failure(exception),_,_,_) => Failure(exception)
    case (_,_,_,_,Failure(exception),_,_) => Failure(exception)
    case (_,_,_,_,_,Failure(exception),_) => Failure(exception)
    case (_,_,_,_,_,_,Failure(exception)) => Failure(exception)
  }
